...
module.exports = {
  ...
  init: function(aParams){
    ...
    this.reloadDB();
    oExec("rhythmbox-client --no-start &");
  },
  ...
  evaluateCommand: function(sCommand, aParams){
    switch(sCommand){
      ...
      case "playPlaylist":
        return this.playPlaylist(aParams);
      case "reloadDB":
  ...
  reloadDB: function(){
    ...
    sData = oFS.readFileSync(this.sHomeDir + "/.local/share/rhythmbox/playlists.xml", "utf-8");
    oXML.parseString(sData, function(oErr, oData){
      if(oErr != null){ console.log(oErr); return; }
      for(var lIndex in oData["rhythmdb-playlists"].playlist){
        if(oData["rhythmdb-playlists"].playlist[lIndex]["$"].type != "static") continue;
        this.oPlaylists[oData["rhythmdb-playlists"].playlist[lIndex]["$"].name] = [];
        for(var lIndexI in oData["rhythmdb-playlists"].playlist[lIndex].location)  this.oPlaylists[oData["rhythmdb-playlists"].playlist[lIndex]["$"].name].push(oData["rhythmdb-playlists"].playlist[lIndex].location[lIndexI]);
      }
    }.bind(this));
  },
  ...
  getPlaylists: function(){
    var aResults = [];
    for(var lIndex in this.oPlaylists) aResults.push(lIndex);
    return aResults;
  },
  ...
  playPlaylist: function(aPlaylist){
    this.clearQueue();
    for(var lIndex in this.oPlaylists[aPlaylist[0]]) this.enqueue(this.oPlaylists[aPlaylist[0]][lIndex]);
    this.next();
  }
